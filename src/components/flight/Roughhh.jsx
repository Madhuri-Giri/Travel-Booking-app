...
export default function FlightLists() {
    ...
    useEffect(() => {
        ...
        // Step 2: Further filter based on selected stop options
        const stopFilteredFlights = filteredFlightsNew.map(segments => {
            return segments.filter((flight) => {
                const segmentsLength = flight.Segments[0].length; // Length of segments
    
                // Check if selected options match the number of stops
                const isStopSelected = selectedOption.includes('stop') && segmentsLength > 1;
                const isNonStopSelected = selectedOption.includes('non-stop') && segmentsLength === 1;
    
                return isStopSelected || isNonStopSelected || selectedOption.length === 0; // Include if no options are selected
            });
        }).filter(segment => segment.length > 0); // Filter out empty segments after stop filtering
    
        // Step 3: Sort the filtered flights based on sortOrder
        const sortedFlights = stopFilteredFlights.map(segments => {
            return segments.sort((a, b) => {
                if (sortOrder === 'lowToHigh') {
                    return a.OfferedFare - b.OfferedFare;
                } else if (sortOrder === 'highToLow') {
        ...
        setfilteredFlights(sortedFlights);
    }, [dd, selectedAirlines, sortOrder, selectedOption]); // Include selectedOptions in dependencies
                                                            ...
                                                            <div className="flightStopsDIV">
                                                                <label className="square-checkbox">
                                                                    <input
                                                                        ...
                                                                        checked={selectedOption.includes('stop')}
                                                            ...
                                                            <div className="flightStopsDIV">
                                                                <label className="square-checkbox">
                                                                    <input
                                                                        ...
                                                                        checked={selectedOption.includes('non-stop')}

...
const BusLists = () => {
  ...
  const { from, to, searchResults, status, error } = useSelector(
    (state) => state.bus
  );
  ...
  useEffect(() => {
    if (!searchResults.length) {
      dispatch(
        searchBuses({ from: "Bhopal", to: "Indore", departDate: "2024-07-26" })
      );
    }
          ...
          <div className="B-lists-Top">
            <div className="text">
              ...
              <div className="search-functinality">
                <button onClick={responsiveFilter} className="filter-bus">
                  <i className="ri-equalizer-line"></i> Filter
                </button>
                <button onClick={navigateSearch}>
                  <i className="ri-pencil-fill"></i>Modify
                </button>
              </div>
            </div>
          </div>
                      ...
                      <div className="fix-trav">
                        <div className="travel-list">
                          {searchResults.map((bus, index) => (
                            <small
                              key={index}
                              onClick={() => setOperatorFilter(bus.TravelName)}
                            >
                              {bus.TravelName}
                            </small>
                          ))}
                            ...
                            <div className="travel-list">
                              {searchResults.map((bus, index) => (
                                <small
                                  key={index}
                                  onClick={() =>
                                    setOperatorFilter(bus.TravelName)
                                  }
                                >
                                  {bus.TravelName}
                                </small>
                        ...
                        <div className="one-top">
                          ...
                          <div className="price">
                            <h4>
                              ₹
                              {Math.round(
                                bus.Price.PublishedPrice * (1 - 0.18)
                              )}
                            </h4>
                            <h6
                              style={{

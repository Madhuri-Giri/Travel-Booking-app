...
const GuestDetails = () => {
  ...
  const NoOfAdults = parseInt(localStorage.getItem('numberOfAdults'), 10);
  ...
  const bookHandler = async () => {
    try {
      ...
      const selectedRoomsData = JSON.parse(localStorage.getItem('selectedRoomsData')) || {};
      const hotelBlockData = JSON.parse(localStorage.getItem('hotelBlock')) || {};
  
      const roomDetails = selectedRoomsData.rooms || [];
      
      if (!Array.isArray(roomDetails)) {
        throw new Error('Invalid roomDetails format');
      }
  
      const bookingPayload = {
        ResultIndex: "9",
        HotelCode: hotelBlockData.hotelCode || "92G|DEL",
        HotelName: hotelBlockData.hotelName || "The Manor",
        GuestNationality: hotelBlockData.guestNationality || "IN",
        NoOfRooms: roomDetails.length || 0,
        ClientReferenceNo: 0,
        IsVoucherBooking: true,
        transaction_num: transactionNum,
        transaction_id: transaction_id,
        hotel_booking_id: hotel_booking_id,
        HotelRoomsDetails: roomDetails.length > 0 ? roomDetails.map((room, index) => ({
          ChildCount: room.childCount || 0,
          RequireAllPaxDetails: room.RequireAllPaxDetails || false,
          RoomId: room.roomId || "",
          RoomStatus: room.roomStatus || "unknown",
          ...
          SupplierPrice: room.SupplierPrice || 0,
          Price: {
        ...
        SrdvType: selectedRoomsData.srdvType || "SingleTB",
        SrdvIndex: selectedRoomsData.srdvIndex || "SrdvTB",
        TraceId: selectedRoomsData.traceId || "1",
      };
